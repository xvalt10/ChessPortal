...
import static application.util.GameUtil.*;
...
public class UserSessionHandler {
    ...
    public void makeMove(JsonObject message) {
        ...
        int utcOffsetSeconds = message.getJsonNumber("utcOffset").intValue();

        LocalDateTime moveDateTime = GameUtil.convertMillisToUTCDateTime(timestamp, utcOffsetSeconds);
        ...
        if (game != null) {
            ...
            if (tournamentId != null && simulHandler.getTournament(tournamentId) != null) {
                Simul simul = (Simul) simulHandler.getTournament(tournamentId);
                if ((whiteMove && game.getWhitePlayer().equals(simul.getSimulOrganizer())) || (!whiteMove && game.getBlackPlayer().equals(simul.getSimulOrganizer()))) {

                    List<String> gamesWaitingForOrganizersMove = simul.getGamesWaitingForMove();
                    gamesWaitingForOrganizersMove.remove(gameId);
                    String nextGameId = null;
                    if (!gamesWaitingForOrganizersMove.isEmpty()) {
                        nextGameId = gamesWaitingForOrganizersMove.stream().findFirst().orElse(null);

                    } else {
                        nextGameId = simul.getGameIds().stream().filter(gameid -> !gameid.equals(gameId)).findFirst().orElse(null);
                    }

                    JsonObjectBuilder jsonObjectBuilder = JsonProvider.provider().createObjectBuilder();
                    JsonObject nextSimulGameMessage = jsonObjectBuilder
                            .add("action", "nextSimulGame")
                            .add("gameId", nextGameId).build();

                    sendMessageToSession(simul.getSimulOrganizer().getSession(), nextSimulGameMessage);

                } else {
                    simul.getGamesWaitingForMove().add(gameId);
                }
            ...
            if (whiteMove) {
                ...
                move.setWhiteMoveTimestamp(moveDateTime);
                ...
                move.setBlackMoveTimestamp(moveDateTime);
                move.setBlackTime(blackTime);
                move.setWhiteTime(whiteTime);
    ...
    public void sendMessageToSession(WebSocketSession session, JsonObject message) {
        sendMessageToSessionWithRetry(session, message, 0);
    }

    public void sendMessageToSessionWithRetry(WebSocketSession session, JsonObject message, int retryAttempts) {
        try {
            ...
            if (retryAttempts <= 3) {
                sendMessageToSessionWithRetry(session, message, ++retryAttempts);
            }
        }
    }
