...
export class TournamentSummaryComponent implements OnInit {
    ...
    constructor(private httpService: HttpService, public gameService: GameService, private router: Router, private formBuilder: FormBuilder, private authService: JwtAuthenticationService) {
        this.loading = false;
    ...
    ngOnInit(): void {
        ...
        this.httpService.getTournamentsByState('NOT_STARTED').subscribe(tournaments => {
            console.log("Scheduled tournaments");
    ...
    scheduleTournament() {
        ...
        tournamentparams['numberOfSimulOponents'] = this.formControls.maxPlayers.value;
        ...
        tournamentparams['organizer'] = this.authService.getUsername();

        if (this.formControls.tournamenttype.value === 'SIMUL') {
            this.httpService.scheduleSimul(tournamentparams).subscribe((response) => {
                    this.handleScheduleFormResponse(response);
                },
                (error) => {
    ...
    private handleScheduleFormResponse(response: Object) {
        this.successMessage = `Event ${this.formControls.tournamentname.value} has been successfully scheduled for ${this.formControls.startDateTime.value}`;
