...
export class JwtAuthenticationService {
    ...
    authenticate(credentials: any) {
        return this.http.post<any>(`${BASEURL}/authenticate`, credentials, { observe: 'response' })
            .pipe(map(response => {
                ...
                let userCountryCode = response.headers.get('countrycode');
                // let user = {};
                ...
                if (userCountryCode) {
                    this.setUserData('countrycode', userCountryCode);
                }
    ...
    getUserData(attribute: string) {
        if (!localStorage.getItem('currentUser')) {
            ...
            return JSON.parse(localStorage.getItem('currentUser'))[attribute];
        }

    }

    setUserData(attributeName: string, attributeValue: string) {
        if (!localStorage.getItem('currentUser')) {
            return;
        } else {
