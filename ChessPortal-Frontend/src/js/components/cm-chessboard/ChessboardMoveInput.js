...
export class ChessboardMoveInput {
    ...
    registerMouseEventListeners(params) {
    ...
    onPointerDown(e) {
        if (e.type === "mousedown" && e.button === 0 || e.type === "touchstart") {
            ...
            const squareElement = this.view.getSquare(index)
            if (squareElement) {
                if (e.altKey) {
                    this.setMoveInputState(STATE.arrowStart, {
                        index: index,
                        type: e.type
                    })
                }
                else if (e.shiftKey) {
                    this.view.setSquareColor(index, 'yellow');
                }
                else if (e.ctrlKey) {
                    this.view.setSquareColor(index, 'red');
                }
            }
            if (index !== undefined) {
                let pieceName, color
    ...
    onPointerUp(e) {
        ...
        if (target && target.getAttribute) {
            ...
            if (index) {
                ...
                } else if(this.moveInputState === STATE.arrowStart){
                        this.view.drawArrow(this.view.squareIndexToPoint(this.startIndex), this.view.squareIndexToPoint(index));
                        this.setMoveInputState(STATE.waitForInputStart);
                }
            } else {
                this.view.drawPiecesDebounced()
